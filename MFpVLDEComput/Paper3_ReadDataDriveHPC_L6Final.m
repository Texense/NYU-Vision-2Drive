...
%% 0: make dirs
function [] = Paper3_ReadDataDriveHPC_L6Final(sampleId)
CurrentFolder = pwd
...
%SampleNum = 10;
aa = 1; bb = length(GratingAll);

BgFrs = zeros(aa*bb,3);
DrvFrs = cell(aa*bb,4); %SCEI
MtpRcd = zeros(aa*bb,1);
...
%             tMod = 1e3/LGNFreq;
            ...
            %% Read dat
            ReadFolder = [SaveToFolder sprintf('L6Final_Ang%.1f_sample%d/',Grating,sampleId)];
            MtpRcd(dirInd,1) = Grating;
            fprintf('L6Final_Ang%.1f_sample%s/',Grating,sampleId)
...
save([SaveToFolder sprintf('Paper3DriveNW_L6_Samps%d_Final.mat',...
    sampleId)],'BgFrs','DrvFrs','MtpRcd')
end