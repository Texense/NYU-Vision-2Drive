...
function [] = Paper3_TestL6ShapeDriveHPC_Final(Grating, varargin)
%% 0: make dirs
CurrentFolder = pwd
...
SaveFolder = [DataFolder sprintf('Paper3NWData/L6Final_Ang%.1f_sample%s/',Grating,NWSampleInd)];
...
load('AllMFPixPara_Paper2TuneFig1V4D2.mat','InEs','InIs','EndState');
...
L6Ord_F_Grating = min(L6Ord_F) + ...
     (max(L6Ord_F)-min(L6Ord_F)) * L6CurveFinalize(ScaledLGN);
...
FigOn = false;
L6smear = n_S_HC*0.33;% was 0.25
TruncL6 = 0.33/(L6smear/n_S_HC); 
L6SFilt = SpatialGaussianFilt_my(OD_SMap,N_HC,n_S_HC,L6smear,TruncL6,FigOn,L6S_Drive);
L6CFilt = SpatialGaussianFilt_my(OD_CMap,N_HC,n_C_HC,L6smear,TruncL6,FigOn,L6C_Drive);
L6IFilt = SpatialGaussianFilt_my(OD_IMap,N_HC,n_I_HC,L6smear,TruncL6,FigOn,L6I_Drive);
...
for TSec = 1:TPar
    ...
    toc
    DriveFName = [sprintf('DriveWkSp_L6Final_Ang%.1f',Grating) num2str(TSec) 's.mat'];
