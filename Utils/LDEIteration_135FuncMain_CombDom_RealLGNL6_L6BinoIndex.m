%% Main diff from the mostly used one: testing L6 binocular index -- something may not include later
% L6BinoIndex: A number between 0 and 1, indicating how much L6 come from
% binocular response, and how much comes from L4 responses
% L4ENESSBino: NESS L4E that we use to compute the binocular L6 response,
% same as output, it should have SCI domains, and the same length as output
...
%% Determine binocular L6 input
LDEBino = L4ENESSBino;
if InhKillFlag % apply inhibition suppresion
    LDEBino.E = LDEBino.S * (1-0.3077) + LDEBino.C * 0.3077;
    LDEBinoEAdj = L6Convert(LDEBino.E,EKp)./LDEBino.E;
    LDEBino.S = LDEBino.S .* LDEBinoEAdj;
    LDEBino.C = LDEBino.C .* LDEBinoEAdj;
end
LDEBino.E = LDEBino.S * (1-0.3077) + LDEBino.C * 0.3077;

% Obtain the L6 input
FieldRow = N_HCOutY * NPixY;
FieldCol = floor(length(LDEBino.E)/FieldRow);
L4EBinofield = reshape(LDEBino.E,FieldRow,FieldCol);
L4EBinofield_padded = padarray(L4EBinofield, [1, 1], 'circular'); % Pad B for periodic boundary conditions
CBino = conv2(L4EBinofield_padded, L6Kernel, 'same'); % Perform convolution with periodic boundary conditions
%% The slope of L6 is adjustable
L6EBino = L6Convert(CBino(2:end-1, 2:end-1),L6pars);% Extract the part of L6 that corresponds to the original dimensions of B

%L61*(C(2:end-1, 2:end-1)-L62);
L6EBino(L6EBino>3*length(L6EMesh)) = 3*length(L6EMesh); L6EBino(L6EBino<3) = 3; % for now, I can't handel fL6>120 Hz since I didn't precompute that
% compute the L6 plane that I should refer to. 3 = layer 1; 120 = layer
% 40, and interpolate the rest.
L6EBinoLibInd = L6EBino(:)/3; % should be 1-40
...
for Epc = 1:Epoc
    ...
    % compute the L6 plane that I should refer to. 3 = layer 1; 120 = layer
    % 40, and interpolate the rest.
    L6EMonoLibInd = L6EUse(:)/3; % should be 1-40
    L6ELibInd = L6EMonoLibInd * (1-L6BinoIndex) + L6EBinoLibInd * L6BinoIndex;
