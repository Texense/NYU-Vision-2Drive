...
function C_SS_mean = AveSpatKer(C_SS_Pixel_Us,N_HCin,N_HCout,NPixX,NPixY)
%% Check if C_SS is an eligible matrix
if size(C_SS_Pixel_Us,1) ~= size(C_SS_Pixel_Us,2)
    error('Input Matrix not square!')
end
if size(C_SS_Pixel_Us,2) ~= N_HCin*NPixX*N_HCin*NPixY
...
for PixInd = 1:N_HCout*NPixX*N_HCout*NPixY
    ...
    if mod(N_HCout,2) == 1
        ...
        ConnMap_Rev(YRange,XRange) = Ker_SS_mean;
